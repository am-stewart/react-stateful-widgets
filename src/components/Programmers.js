...
const [pro, setPro] = useState(null);
  ...
  const getNameOfFeatured = () => {
    ...
    return list[pro-1].name;
  };

  const style = {
    fontSize: '1.5em',
    marginTop: '0.5em',
    color: !pro ? 'royalblue' : 'gold', // 🤔 color turns to gold, when celebrating
  };
      ...
      <div className='programmers'>
        {
          ...
          list.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.name} <button onClick={() => {setPro(dev.id)/* in here set the featured id to be dev.id */ }}>Feature</button>
            </div>
          )
        }
      </div>
      <div id='featured' style={style}>
        {
          ...
          pro ? `🎉 Let's celebrate ${getNameOfFeatured()}! 🥳` : 'Pick an awesome programmer'
        }
      </div>
    </div>
  );
